...
- Per-collection regressions trigger a FALLBACK, not a refusal: the collection is frozen at its published content (files carried from the clone into the snapshot, counts restored to the previous manifest values) and the rest of the snapshot publishes normally. The manifest records it in `fallbacks`, the log raises an ERROR. Totals (acts/articles/links across the whole corpus) stay fail-closed: if everything regresses, publication is refused. Detection happens pre-render by comparing distinct candidate URNs per collection against the previous manifest's `converted`.
